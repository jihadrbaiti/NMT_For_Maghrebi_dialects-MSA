...
from data import *
from models.model.transformer_enc import Transformer
...
print()
print('***************BERT in Encoder - Maghrebi dialects to MSA -***************')
print()
...
def run(total_epoch, best_loss):
    ...
    for step in range(total_epoch):
        ...
        if valid_loss < best_loss:
            ...
            torch.save(model.state_dict(), './saved/N2M/saved_enc/model-{0}.pt'.format(valid_loss))

        f = open('./results_final/N2M/result_enc/train_loss.txt', 'w+')
        f.write(str(train_losses))
        f.close()

        f = open('./results_final/N2M/result_enc/bleu.txt', 'w+')
        f.write(str(bleus))
        f.close()

        f = open('./results_final/N2M/result_enc/bleu2.txt', 'w+')
        f.write(str(bleus2))
        f.close()

        f = open('./results_final/N2M/result_enc/ter.txt', 'w+')
        f.write(str(ters))
        f.close()

        f = open('./results_final/N2M/result_enc/chrf.txt', 'w+')
        f.write(str(chrfs))
        f.close()

        f = open('./results_final/N2M/result_enc/test_loss.txt', 'w+')
    ...
    #torch.save(model.state_dict(), '/home/jihad.rbaiti/lustre/pt_cloud-muhqxqc6fxo/users/jihad.rbaiti/N2M/prepro/saved_v2f_r/saved_config/model-{0}.pt'.format(valid_loss))
