...
model = Transformer(src_pad_idx=src_pad_idx,
                    ...
                    n_layers=n_layers,
                    drop_prob=drop_prob,
                    device=device, bert=DarijaBert_model).to(device)
print()
print('*************** BERT in Decoder - MSA to Maghrebi dialects - ***************')
print()
...
def run(total_epoch, best_loss):
    ...
    for step in range(total_epoch):
        ...
        if valid_loss < best_loss:
            ...
            torch.save(model.state_dict(), './saved/M2N/saved_dec/model-{0}.pt'.format(valid_loss))

        f = open('./results_final/M2N/result_dec/train_loss.txt', 'w+')
        f.write(str(train_losses))
        f.close()

        f = open('./results_final/M2N/result_dec/bleu.txt', 'w+')
        f.write(str(bleus))
        f.close()

        f = open('./results_final/M2N/result_dec/bleu2.txt', 'w+')
        f.write(str(bleus2))
        f.close()

        f = open('./results_final/M2N/result_dec/ter.txt', 'w+')
        f.write(str(ters))
        f.close()

        f = open('./results_final/M2N/result_dec/chrf.txt', 'w+')
        f.write(str(chrfs))
        f.close()

        f = open('./results_final/M2N/result_dec/test_loss.txt', 'w+')
