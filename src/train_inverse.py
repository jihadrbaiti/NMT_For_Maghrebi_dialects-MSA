...
print()
print('***************  Our proposed approach - MSA to Maghrebi dialects - ***************')
print()
...
def run(total_epoch, best_loss):
    ...
    for step in range(total_epoch):
        ...
        if valid_loss < best_loss:
            ...
            torch.save(model.state_dict(), './saved/M2N/saved/model-{0}.pt'.format(valid_loss))

        f = open('./results_final/M2N/result_f/train_loss.txt', 'w+')
        f.write(str(train_losses))
        f.close()

        f = open('./results_final/M2N/result_f/bleu.txt', 'w+')
        f.write(str(bleus))
        f.close()

        f = open('./results_final/M2N/result_f/bleu2.txt', 'w+')
        f.write(str(bleus2))
        f.close()

        f = open('./results_final/M2N/result_f/ter.txt', 'w+')
        f.write(str(ters))
        f.close()

        f = open('./results_final/M2N/result_f/chrf.txt', 'w+')
        f.write(str(chrfs))
        f.close()

        f = open('./results_final/M2N/result_f/test_loss.txt', 'w+')
